...
class AMPPPO(PPO):
    ...
    def update(self) -> dict[str, float]:
        return self._joint_update()

    def _joint_update(self) -> dict[str, float]:
        mean_value_loss = 0.0
    ...
    def _load_split_optimizer_states(self, policy_state: dict, discriminator_state: dict) -> None:
        ...
        old_discriminator_ids = discriminator_state["param_groups"][0]["params"]
        new_discriminator_ids = merged["param_groups"][1]["params"] + merged["param_groups"][2]["params"]
        for old_id, new_id in zip(old_discriminator_ids, new_discriminator_ids):
